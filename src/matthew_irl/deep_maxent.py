...
def irl(structure, feature_matrix, n_actions, discount, transition_probability,
    ...
    # Initialise alpha weight, beta bias.
    if initialisation == "normal":
    ...
    phis = [s_feature_matrix.T]
    # Forward propagation.
    for W, b in zip(weights, biases):
        phi = T.nnet.sigmoid(th.compile.ops.Rebroadcast((0, False), (1, True))(b)
                           + W.dot(phis[-1]))
        phis.append(phi)
        # phis[1] = phi1 etc.
    # Reward.
    r = alpha.dot(phis[-1]).reshape((n_states,))
    ...
    updates = []
    alpha_grad = phis[-1].dot(svf - expected_svf).T
    hist_alpha_grad += alpha_grad**2
    adj_alpha_grad = alpha_grad/(adagrad_epsilon + T.sqrt(hist_alpha_grad))
